...
class Character( Thing ):
	...
	def takeDamage( self, amt, cannotDie = False ):
		...
		if self.hp < 0:
			#don't die if you're taking damage from your own attack
			if cannotDie == True: 
				self.hp = 1 # still keep 1 hp
			else: 
				self.hp = 0 #if you're dying, set hp to 0, not negative number
...
class PlayableCharacter( Character ):
	...
	def __init__( self, pos, battlePos, imglist, name, namePos, stagePos = None ):
		...
		#as well as original stats pre-boost
		self.boostedStats = [] 
...
# 				battleDamage = battle[2:] # last two images are for taking damage
	...
	# updates temporary stats in battle
