...
class AttackChooser:
	def __init__(self, parSurface):
		...
		self.atkText = self.font.render(" ", True, white)
		self.descText = self.font.render(" ", True, white)
	...
	def config(self, chara):
		...
		#blit permanent moves in red 
		if name == "Cancel Plans" or name == "Flee":
			self.atkText = self.font.render(name, True, red)
		else:
			self.atkText = self.font.render(name, True, blue)

		self.descText = self.descFont.render(self.attack().desc, True, white)
	...
	def draw(self): # too much rendering ( in most cases, only text needs updating )
		...
		# draw name
		name = self.nameFont.render(self.chara.name, True, green)
