...
class Conversation():
	def __init__(self, textboxImg, textboxCoord, cursor, window, charList, convoFont, nameFont, textFile):
		...
		self.upperLeft = [self.winWidth-self.boxWidth+10, self.winHeight-self.boxHeight+80]
		self.nameBoxCenterLeft = [self.boxX+18, self.boxY-44]

		self.cursorCoords = (self.boxX + self.boxWidth - 30, self.boxY + self.boxHeight -cursor.get_height())
		...
		if len(self.allConvos[convoNum])<(self.currentBoxIdx*2)+1:
			...
			for char in self.charList:
				if char.name == name:
					...
					nameCoords = self.nameBoxCenterLeft
	...
	def makeTextboxes(self, conversations):
		...
		for conv in convList:
			for personTurn in conv:
				...
				blitList = ["","",""]
				for blitChar in wordsAndSpacesList[1:]:
